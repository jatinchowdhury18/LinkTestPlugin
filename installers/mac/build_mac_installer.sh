...
script_file=LinkTestPlugin.pkgproj
...
pkg_dir=LinkTestPlugin_Installer_Packaged
rm -Rf $pkg_dir
mkdir $pkg_dir
productsign -s "$TEAM_ID" ../../build/LinkTestPlugin.pkg $pkg_dir/LinkTestPlugin-signed.pkg

echo "Notarizing installer package..."
INSTALLER_PASS=$(more ~/Developer/mac_installer_pass)
npx notarize-cli --file $pkg_dir/LinkTestPlugin-signed.pkg --bundle-id com.chowdsp.LinkTestPlugin --asc-provider "$TEAM_ID" --username chowdsp@gmail.com --password "$INSTALLER_PASS"

echo "Building disk image..."
vol_name=Install_LinkTestPlugin-$app_version
rm -f "$vol_name"

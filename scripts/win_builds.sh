...
mkdir -p bin/Win64
mkdir -p bin/Win32
declare -a plugins=("LinkTestPlugin")
...
(
    cd bin
    rm -f "LinkTestPlugin-Win64-${VERSION}.zip"
    zip -r "LinkTestPlugin-Win64-${VERSION}.zip" Win64
    # rm -f "LinkTestPlugin-Win32-${VERSION}.zip"
    # zip -r "LinkTestPlugin-Win32-${VERSION}.zip" Win32
)
